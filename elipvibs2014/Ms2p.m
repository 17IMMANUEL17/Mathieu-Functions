...
if mod(m,2)==0 % even order
  ...
  %   (p0*cos(k*pi)'.*u);
  f1p=z1p.*(besp(k-s,z1).*besy(k+s,z2)-...
            besp(k+s,z1).*besy(k-s,z2));
  f2p=z2p.*(bes(k-s,z1).*besyp(k+s,z2)-...
            bes(k+s,z1).*besyp(k-s,z2));
  ...
  %   (p0*cos(k*pi)'.*u);
  f1p=z1p.*(besp(k-s,z1).*besy(k+s+1,z2)-...
            besp(k+s+1,z1).*besy(k-s,z2));
  f2p=z2p.*(bes(k-s,z1).*besyp(k+s+1,z2)-...
            bes(k+s+1,z1).*besyp(k-s,z2));
  fp=(f1p+f2p)*(p0*cos(k*pi)'.*u);      
end
