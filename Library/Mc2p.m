function fp=Mc2p(z,m,q,ntrms)
% fp=Mc2p(z,m,q,ntrms)

% Derivative with respect to z of the
% modified Mathieu function Mc2(z,m,q)
...
if mod(m,2)==0 % even index   
  ...
  %   (p0.*u);
  f1p=u1p.*(besp(k-s,u1).*besy(k+s,u2)+...
            besp(k+s,u1).*besy(k-s,u2));
  f2p=u2p.*(bes(k-s,u1).*besyp(k+s,u2)+...
            bes(k+s,u1).*besyp(k-s,u2));
  fp=(f1p+f2p)*(p0.*u);      
else           % odd index
  ...
  %   (p0.*u); 
  f1p=u1p.*(besp(k-s,u1).*besy(k+s+1,u2)+...
            besp(k+s+1,u1).*besy(k-s,u2));
  f2p=u2p.*(bes(k-s,u1).*besyp(k+s+1,u2)+...
            bes(k+s+1,u1).*besyp(k-s,u2));
  fp=(f1p+f2p)*(p0.*u);      
end
...
function f=besy(n,z)
% f=besy(n,z) computes bessely for vector
% values of n and z
f= bessely(ones(size(z(:)))*n(:)',z(:)*ones(size(n(:)'))); 

%========================================

function fp=besyp(n,z)
% fp=besyp(n,z) computes the derivative of 
% bessely for vector values of n and z
n=n(:)'; z=z(:);
fp=(besy(n-1,z)-besy(n+1,z))/2;