...
if mod(m,2)==0 % even index   
  ...
  f=(bes(k-s,u1).*besy(k+s,u2)+...
     bes(k+s,u1).*besy(k-s,u2))*...
     (p0.*u);
else           % odd index
  [a,c]=matue(q,2,1,ntrms); r1=(m+1)/2;
  u=c(:,mh); [ubig,s]=max(abs(u));
  p0=-(-1)^r1/u(s)*sgn; s=s-1;
  f=(bes(k-s,u1).*besy(k+s+1,u2)+...
     bes(k+s+1,u1).*besy(k-s,u2))*...
     (p0.*u); 
...
function f=besy(n,z)
% f=besy(n,z) calls function bessely
% for vector values of n and z.
