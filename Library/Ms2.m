function f=Ms2(z,m,q,ntrms)
% f=Ms2(z,m,q,ntrms)

% Modified Mathieu function Ms2(z,m,q)
...
if mod(m,2)==0 % even order
  ...
  f=(bes(k-s,z1).*besy(k+s,z2)-...
     bes(k+s,z1).*besy(k-s,z2))*...
     (p0*cos(k*pi)'.*u);
else           % odd order
  k=0:ntrms-1; [a,c]=matue(q,2,2,ntrms); 
  u=c(:,1+fix(m/2)); [ubig,s]=max(abs(u));
  r=(m-1)/2; p0=(-1)^r/u(s); s=s-1;
  f=(bes(k-s,z1).*besy(k+s+1,z2)-...
     bes(k+s+1,z1).*besy(k-s,z2))*...
     (p0*cos(k*pi)'.*u);
